...
export const MapScreen: React.FC = () => {
  ...
  const completedScenarios = MAP_ZONES.reduce((sum, z) => sum + z.scenarioIds.filter(id => playedScenarioIds.has(id)).length, 0);

  return (
    <div className="flex flex-col h-[calc(100vh-95px)] overflow-hidden bg-[#1a3a5c] relative">
      {/* MAP AREA */}
      <div className="flex-1 relative overflow-hidden" ref={mapRef} onClick={handleCloseDetail}>
        ...
        {/* Zoom controls */}
        <div className="absolute top-2 right-2 md:top-4 md:right-4 flex flex-col gap-1.5 md:gap-2 z-30">
          {[
            ...
            <button key={btn.icon} onClick={(e) => { e.stopPropagation(); btn.action(); }} className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-white/90 shadow-md flex items-center justify-center text-[#003e6f] hover:bg-white transition-colors">
              <span className="material-symbols-outlined text-[18px] md:text-[20px]">{btn.icon}</span>
            </button>
          ))}
        </div>

        {/* Progress badge (top-left, smaller on mobile) */}
      ...
      {/* ZONE DETAIL — mobile: slide-up drawer, desktop: left-bottom card */}
      {selectedZone && showDetail && (
        <>
          <div className="md:hidden fixed inset-0 z-25 bg-black/40" onClick={handleCloseDetail} />
          <div
            className="z-30 bg-white/95 backdrop-blur-md rounded-t-2xl md:rounded-2xl shadow-xl overflow-hidden animate-in md:absolute md:bottom-4 md:left-4 md:w-[320px] md:max-w-[calc(100%-2rem)] fixed bottom-0 left-0 right-0 max-h-[55vh] md:max-h-none flex flex-col slide-in-from-bottom"
      ...
      {/* Bottom quick-nav strip (horizontal scroll) */}
